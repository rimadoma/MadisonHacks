...
public class ThresholdVolumeFractionTest {
	...
	public void testThresholdVolumeFractionFailsMatchWith2DImage() {
		// to be called unary the 2nd argument for this BinaryFunction has to be
		// set non null in the matcher
		Img img = (Img) IMAGE_J.op().run(CreateImgFromDimsAndType.class, new FinalDimensions(10, 10), new LongType());
		final Settings settings = new Settings(1, 1, 100);
		IMAGE_J.op().op(ThresholdVolumeFraction.class, img, settings);
	}

	@Test
	public void regressionTestUnitCube() throws AssertionError {
		/*
		 * The surface created by the marching cubes algorithm in
		 * ThresholdVolumeFraction is " in between pixels". In the case of a
		 * (BitType) unit cube it creates an octahedron, whose vertices are in
		 * the middle of the faces of the cube.
		 */
		final long width = 1L;
		...
		final IterableInterval<BitType> unitCube = (IterableInterval<BitType>) IMAGE_J.op().run(CuboidCreator.class,
				null, width, height, depth);
		final Settings settings = new Settings(1.0, 1.0, 1.0);

		final Results results = (Results) IMAGE_J.op().run(ThresholdVolumeFraction.class, unitCube, settings);
	...
	/**
	 * Verify that if half of the foreground elements in an image are within the
	 * thresholds, then the volume of the thresholdMesh is (about) half of the
	 * foregroundMesh
	 */
	@Test
	public void regressionTestThresholdMeshVolume() throws AssertionError {
		...
		final Img<LongType> testImg = (Img<LongType>) IMAGE_J.op().run(CreateImgFromDimsAndType.class,
				new FinalDimensions(uSize, vSize, wSize), new LongType());
		fillWithThirdDimGradient(testImg);

		final Results results = (Results) IMAGE_J.op().run(ThresholdVolumeFraction.class, testImg, settings);

		// Volume will not be exactly half, because marching cubes will "round"
		// / "smooth" the edges
		// of the mesh created from elements within the thresholds.
		// Otherwise this mesh would be the cuboid cut in half across the 3rd
		// dimension plane
