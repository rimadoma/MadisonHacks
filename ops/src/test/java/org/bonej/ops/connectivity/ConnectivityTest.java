 ...
 * @todo write a tests with images that touche the interval boundaries to
 *       properly test deltaChi
 */
public class ConnectivityTest {
	...
	/**
	 * Test that the Connectivity Op gets matched with correct input. Calling
	 * Connectivity.conforms() is a part of the matching process
	 */
	@Test
	public void testConnectivityMatchesWith3DImage() {
		final double[] calibration = {0.2, 0.2, 0.2};
		Object cuboid = IMAGE_J.op().run(WireFrameCuboidCreator.class, null, 10, 10, 10, 0, calibration);

		Connectivity connectivity = IMAGE_J.op().op(Connectivity.class, cuboid);
	...
	public void testConnectivityFailsMatchWith2DImage() {
		...
		IMAGE_J.op().op(Connectivity.class, testImage);
	}
	...
	public void regressionTestConnectivityDensity() {
		...
		ImgPlus<BitType> cuboid = (ImgPlus<BitType>) IMAGE_J.op().run(WireFrameCuboidCreator.class, null, CUBOID_WIDTH,
				CUBOID_HEIGHT, CUBOID_DEPTH, PADDING, CALIBRATION);

		Connectivity.Characteristics results = (Connectivity.Characteristics) IMAGE_J.op().run(Connectivity.class,
				cuboid);
